...
| TD-023 | Non-atomic default-filter-set race in `POST /api/user/filters` and `PATCH /api/user/filters/[id]` | Setting a filter set as default is two separate round-trips — clear the old default, then insert/update the new one. A mid-flight failure between the two leaves a user with zero defaults. A fix already exists on the unmerged `fix/default-filter-race-and-auth-improvements` branch (commit `4cc0f10`): atomic Postgres RPC functions `insert_default_filter_set`/`set_default_filter_set` replacing the two-step pattern. **Needs adapting before reuse**: that commit's RPC SQL types `p_user_id` as `uuid`; post-Clerk-migration `saved_filter_sets.user_id` is `TEXT`, so the RPC signature and its SQL migration need updating to match. Also needs its own manual Supabase SQL Editor migration step | Unassigned | Open |
