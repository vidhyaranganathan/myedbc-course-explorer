...
| TD-022 | Flaky default-filter auto-load test | Resolved: `"auto-loads the default filter set on mount when logged in"` (`src/app/page.test.tsx`) raced two independent `useEffect`s in `page.tsx` — one renders the unfiltered course list from `/api/courses`, the other chains `/api/auth/me` → `/api/user/filters` to apply the default saved filter set. **Fixed twice**: the original fix awaited `findByText("Science 11")` then asserted `"Mathematics 10"` absence synchronously — not airtight, since "Science 11" is also in the unfiltered list, so the `findByText` could resolve before the filter effect ran, leaving the synchronous absence check racy. Stress-testing on 2026-08-17 (while adding unrelated saved-filter-set tests) reproduced the race directly. Final fix wraps both assertions in one `waitFor(...)` that polls the conjunction instead of snapshotting it after a single `findByText`. 15/15 clean on repeated stress runs | Unassigned | Resolved |
