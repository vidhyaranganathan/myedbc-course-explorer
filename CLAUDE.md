...
- `docs/exploreuserdb/` — User DB design exploration (not yet implemented). Covers the `profiles` and `saved_filter_sets` schema, RLS policies, filter sharing via `share_token`, encryption decisions, and open questions. See `scripts/user-schema.sql` to run in Supabase SQL Editor when ready.
